...
from .models import Bahia, Reserva, Servicio, MedioPago, DisponibilidadHoraria, HorarioDisponible, Recompensa
from .forms import BahiaForm, ServicioForm, MedioPagoForm, DisponibilidadHorariaForm, ReservaForm, ClienteForm, HorarioDisponibleForm, RecompensaForm
...
class RecompensaListView(LoginRequiredMixin, AdminRequiredMixin, View):
    """Vista para listar todas las recompensas"""
    template_name = 'reservas/recompensa_list.html'

    def get(self, request):
        recompensas = Recompensa.objects.all().order_by('nombre')
        return render(request, self.template_name, {'recompensas': recompensas})


class RecompensaCreateView(LoginRequiredMixin, AdminRequiredMixin, View):
    """Vista para crear una nueva recompensa"""
    template_name = 'reservas/recompensa_form.html'

    def get(self, request):
        form = RecompensaForm()
        return render(request, self.template_name, {
            'form': form,
            'titulo': 'Crear Nueva Recompensa'
        })
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            return redirect('reservas:recompensa_list')
        return render(request, self.template_name, {
            'form': form,
            'titulo': 'Crear Nueva Recompensa'
        })
...
class RecompensaUpdateView(LoginRequiredMixin, AdminRequiredMixin, View):
    """Vista para actualizar una recompensa existente"""
    template_name = 'reservas/recompensa_form.html'
    ...
    def get(self, request, pk):
        ...
        return render(request, self.template_name, {
            ...
            'titulo': f'Editar Recompensa: {recompensa.nombre}'
        })
    ...
    def post(self, request, pk):
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Recompensa actualizada exitosamente')
            return redirect('reservas:recompensa_list')
        return render(request, self.template_name, {
            'form': form,
            'recompensa': recompensa,
...
class RecompensaDeleteView(LoginRequiredMixin, AdminRequiredMixin, View):
    """Vista para eliminar una recompensa"""
    template_name = 'reservas/recompensa_confirm_delete.html'

    def get(self, request, pk):
        recompensa = get_object_or_404(Recompensa, pk=pk)
        return render(request, self.template_name, {'recompensa': recompensa})

    def post(self, request, pk):
        recompensa = get_object_or_404(Recompensa, pk=pk)
        nombre = recompensa.nombre
        recompensa.delete()
        messages.success(request, f'Recompensa {nombre} eliminada exitosamente')
        return redirect('reservas:recompensa_list')
