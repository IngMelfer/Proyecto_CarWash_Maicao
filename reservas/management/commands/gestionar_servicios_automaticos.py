...
2. Finaliza automáticamente servicios en proceso después de 10 minutos de iniciado el servicio.
...
Opciones:
    ...
    --minutos-fin: Tiempo en minutos para finalizar automáticamente un servicio (default: 10)
"""
...
class Command(BaseCommand):
    ...
    y finaliza automáticamente servicios en proceso después de 10 minutos de iniciados.
    """
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=10,
            help='Tiempo en minutos para finalizar automáticamente un servicio (por defecto: 10)',
        )
    ...
    def handle(self, *args, **options):
        ...
        2. Reservas en proceso que llevan más de 10 minutos desde su inicio y las finaliza.
    ...
    def _finalizar_servicios_automaticos(self, dry_run, minutos_fin, now):
        """Finaliza automáticamente servicios en proceso después de un tiempo.
        
        Args:
            dry_run: Si es True, solo simula las acciones sin realizarlas
            minutos_fin: Minutos después de los cuales finalizar automáticamente un servicio
            now: Fecha y hora actual
        """
        # Calcular el tiempo límite para finalizar servicios
        tiempo_limite_fin = now - timedelta(minutes=minutos_fin)
        
        # Buscar reservas en proceso que tienen fecha de inicio y llevan más del tiempo límite
        reservas_por_finalizar = Reserva.objects.filter(
            estado=Reserva.EN_PROCESO,
            fecha_inicio_servicio__lt=tiempo_limite_fin
        )
        
        # Contar cuántas reservas se encontraron
        count_fin = reservas_por_finalizar.count()
        self.stdout.write(self.style.SUCCESS(f'Se encontraron {count_fin} reservas en proceso con más de {minutos_fin} minutos desde su inicio'))
        ...
        for reserva in reservas_por_finalizar:
            if dry_run:
                self.stdout.write(f'[SIMULACIÓN] Finalizando automáticamente la reserva {reserva.id} de {reserva.cliente} - {reserva.servicio}')
            else:
                # Finalizar el servicio
                if reserva.completar_servicio():
                    self.stdout.write(self.style.SUCCESS(f'Reserva {reserva.id} de {reserva.cliente} - {reserva.servicio} finalizada automáticamente'))
                    ...
                    Notificacion.objects.create(
                        ...
                        mensaje=f'Su servicio {reserva.servicio.nombre} ha sido completado automáticamente. ¡Gracias por confiar en nosotros!',
                    )
                else:
        ...
        if not dry_run and count_fin > 0:
            self.stdout.write(self.style.SUCCESS(f'Proceso completado. Se finalizaron {count_fin} servicios automáticamente.'))
        elif count_fin > 0:
